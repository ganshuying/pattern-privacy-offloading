...
class MECEnv(gym.Env):
    ...
    def reset(self):
        ...
        self.current_step = 0
        self.count_wrong = 0
        generate_num, insert_num = self.generate_task()

        # 记录生成数据
        self.task_generate = self.task_generate + generate_num
        # self.request_overflow_num = self.request_overflow_num + (generate_num - insert_num)
    ...
    def step(self, action):
        ...
        ratio = 0
        #
        if self.task_generate==0:
            ratio = 0
        else:
            ratio = self.request_overflow_num / self.task_generate

        #print("fail ", self.request_overflow_num)
       # print("sum ", self.task_generate)
    ...
    def generate_task(self):
        # 任务随机到达
        self.generate_task_prob = np.array([self.task_generation_rate] * self.num_device)
        ...
        return len(new_task), insert_num
...
def test_greedy():
    ...
    for i_episode in range(num_episodes):
        ...
        total_reward = (total_reward / env.task_count) * 500 * env.task_generation_rate
